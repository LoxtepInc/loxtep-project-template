# AGENTS.md — Student Activity Tracker

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template education`. It tells an AI coding agent (Claude Code,
...
Track student engagement and flag at-risk students

This project was scaffolded from the **education** template. It defines
...
.loxtep/
  ...
  skills/education-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/education-default.yaml`. It grants **read-only** access scoped to
...
scope:        # permitted resource identifiers, per type
  data_products: ["student-activities-sink"]
  connectors: ["student-activities-webhook"]
  workflows: ["student-activities-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **education** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `student-activities-sink`
- **Connectors / connections:**
  - `student-activities-webhook`
- **Workflows:**
  - `student-activities-ingestion`
