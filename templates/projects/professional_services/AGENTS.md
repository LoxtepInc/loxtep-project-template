# AGENTS.md — Time Entry Tracker

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template professional_services`. It tells an AI coding agent (Claude Code,
...
Track billable hours and utilization across projects and clients

This project was scaffolded from the **professional_services** template. It defines
...
.loxtep/
  ...
  skills/professional-services-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/professional-services-default.yaml`. It grants **read-only** access scoped to
...
scope:        # permitted resource identifiers, per type
  data_products: ["time-entries-sink"]
  connectors: ["time-entries-webhook"]
  workflows: ["time-entries-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **professional_services** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `time-entries-sink`
- **Connectors / connections:**
  - `time-entries-webhook`
- **Workflows:**
  - `time-entries-ingestion`
