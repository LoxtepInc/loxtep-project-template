# AGENTS.md — Cryptocurrency Price Tracker

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template financial_services`. It tells an AI coding agent (Claude Code,
...
Monitor real-time cryptocurrency prices and flag significant price movements

This project was scaffolded from the **financial_services** template. It defines
...
.loxtep/
  ...
  skills/financial-services-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/financial-services-default.yaml`. It grants **read-only** access scoped to
...
scope:        # permitted resource identifiers, per type
  data_products: ["crypto-prices-sink"]
  connectors: ["crypto-prices-poller"]
  workflows: ["crypto-prices-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **financial_services** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `crypto-prices-sink`
- **Connectors / connections:**
  - `crypto-prices-poller`
- **Workflows:**
  - `crypto-prices-ingestion`
