# AGENTS.md — Public Health Metrics Monitor

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template healthcare`. It tells an AI coding agent (Claude Code,
...
Track population health trends from CDC public health data

This project was scaffolded from the **healthcare** template. It defines
...
.loxtep/
  ...
  skills/healthcare-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/healthcare-default.yaml`. It grants **read-only** access scoped to
...
scope:        # permitted resource identifiers, per type
  data_products: ["health-metrics-sink"]
  connectors: ["health-metrics-preloaded"]
  workflows: ["health-metrics-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **healthcare** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `health-metrics-sink`
- **Connectors / connections:**
  - `health-metrics-preloaded`
- **Workflows:**
