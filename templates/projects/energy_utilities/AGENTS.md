# AGENTS.md — Weather Impact Monitor

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template energy_utilities`. It tells an AI coding agent (Claude Code,
...
Monitor weather conditions and correlate with energy demand

This project was scaffolded from the **energy_utilities** template. It defines
...
.loxtep/
  ...
  skills/energy-utilities-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/energy-utilities-default.yaml`. It grants **read-only** access scoped to
**only** the resources this template defines. Anything not listed there is
denied by default.

```yaml
name: energy-utilities-default
scope:        # permitted resource identifiers, per type
  data_products: ["weather-conditions-sink"]
  connectors: ["weather-data-poller"]
  workflows: ["weather-data-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **energy_utilities** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `weather-conditions-sink`
