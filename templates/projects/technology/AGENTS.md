# AGENTS.md — GitHub Activity Tracker

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template technology`. It tells an AI coding agent (Claude Code,
...
Track public GitHub events to analyze developer activity and open-source trends

This project was scaffolded from the **technology** template. It defines
...
.loxtep/
  ...
  skills/technology-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/technology-default.yaml`. It grants **read-only** access scoped to
...
scope:        # permitted resource identifiers, per type
  data_products: ["github-activity-sink"]
  connectors: ["github-events-poller"]
  workflows: ["github-events-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **technology** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `github-activity-sink`
