# AGENTS.md — Blank Project

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template blank`. It tells an AI coding agent (Claude Code,
...
Create from scratch with no pre-loaded content.

This project was scaffolded from the **blank** template. It defines
0 resources across data
...
.loxtep/
  ...
  skills/blank-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/blank-default.yaml`. It grants **read-only** access scoped to
...
These are the resources defined by the **blank** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - _(none defined yet — add resources, then run `loxtep generate`)_
