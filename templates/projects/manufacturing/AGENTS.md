# AGENTS.md — IoT Sensor Monitoring

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template manufacturing`. It tells an AI coding agent (Claude Code,
...
Monitor equipment sensors for anomalies and threshold violations

This project was scaffolded from the **manufacturing** template. It defines
...
.loxtep/
  ...
  skills/manufacturing-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/manufacturing-default.yaml`. It grants **read-only** access scoped to
**only** the resources this template defines. Anything not listed there is
denied by default.

```yaml
name: manufacturing-default
scope:        # permitted resource identifiers, per type
  data_products: ["sensor-readings-sink"]
  connectors: ["sensor-readings-webhook"]
  workflows: ["sensor-readings-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **manufacturing** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `sensor-readings-sink`
- **Connectors / connections:**
  - `sensor-readings-webhook`
- **Workflows:**
