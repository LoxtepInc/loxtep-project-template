# AGENTS.md — Property Listings Tracker

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template real_estate`. It tells an AI coding agent (Claude Code,
...
Analyze property listings and market trends

This project was scaffolded from the **real_estate** template. It defines
...
.loxtep/
  ...
  skills/real-estate-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/real-estate-default.yaml`. It grants **read-only** access scoped to
...
scope:        # permitted resource identifiers, per type
  data_products: ["property-listings-sink"]
  connectors: ["property-listings-preloaded"]
  workflows: ["property-listings-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **real_estate** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `property-listings-sink`
- **Connectors / connections:**
  - `property-listings-preloaded`
- **Workflows:**
  - `property-listings-ingestion`
