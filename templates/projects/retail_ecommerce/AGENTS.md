# AGENTS.md — Full-Mesh Ecommerce Demo

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template retail_ecommerce`. It tells an AI coding agent (Claude Code,
...
Ecommerce sample with ingestion (webhook → raw orders), enrichment (raw → enriched orders), and consumption (enriched orders → partner webhook). Demonstrates all three workflow types.

This project was scaffolded from the **retail_ecommerce** template. It defines
6 resources across data
...
.loxtep/
  ...
  skills/retail-ecommerce-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/retail-ecommerce-default.yaml`. It grants **read-only** access scoped to
**only** the resources this template defines. Anything not listed there is
denied by default.

```yaml
name: retail-ecommerce-default
scope:        # permitted resource identifiers, per type
  data_products: ["enriched-orders-sink","raw-orders-sink","raw-orders-source"]
  connectors: ["orders-webhook"]
  workflows: ["orders-enrichment","orders-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **retail_ecommerce** template (referenced by
