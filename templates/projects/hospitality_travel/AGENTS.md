# AGENTS.md — Flight Status Tracker

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template hospitality_travel`. It tells an AI coding agent (Claude Code,
...
Track flight delays and calculate on-time performance

This project was scaffolded from the **hospitality_travel** template. It defines
...
.loxtep/
  ...
  skills/hospitality-travel-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/hospitality-travel-default.yaml`. It grants **read-only** access scoped to
**only** the resources this template defines. Anything not listed there is
denied by default.

```yaml
name: hospitality-travel-default
scope:        # permitted resource identifiers, per type
  data_products: ["flight-status-sink"]
  connectors: ["flight-status-preloaded"]
  workflows: ["flight-status-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **hospitality_travel** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `flight-status-sink`
- **Connectors / connections:**
  - `flight-status-preloaded`
- **Workflows:**
