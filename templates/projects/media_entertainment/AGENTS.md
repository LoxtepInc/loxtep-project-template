# AGENTS.md — News Headlines Tracker

> Agent-ready context for this Loxtep project. This file is scaffolded by
> `loxtep init --template media_entertainment`. It tells an AI coding agent (Claude Code,
...
Track news articles and analyze content performance

This project was scaffolded from the **media_entertainment** template. It defines
...
.loxtep/
  ...
  skills/media-entertainment-default.yaml  # default scoped Skill (see "Skill scope" below)
...
`.loxtep/skills/media-entertainment-default.yaml`. It grants **read-only** access scoped to
**only** the resources this template defines. Anything not listed there is
denied by default.

```yaml
name: media-entertainment-default
scope:        # permitted resource identifiers, per type
  data_products: ["news-articles-sink"]
  connectors: ["news-articles-poller"]
  workflows: ["news-articles-ingestion"]
  domains: []
  queues: []
...
These are the resources defined by the **media_entertainment** template (referenced by
their template slugs until `loxtep generate` resolves them to platform ids):

- **Data products:**
  - `news-articles-sink`
- **Connectors / connections:**
  - `news-articles-poller`
- **Workflows:**
